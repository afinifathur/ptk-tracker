  ...
  <div class="max-w-7xl mx-auto p-6">
    ...
    <header class="flex items-center justify-between mb-6">
      ...
      <nav class="topnav hidden md:flex space-x-4">
        ...
        @can('menu.queue')
          <a href="{{ route('ptk.queue') }}"
            ...
            @if(isset($approvalQueueCount) && $approvalQueueCount > 0)
              <span
                class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-semibold rounded-full text-white bg-red-600">
                {{ $approvalQueueCount }}
              </span>
            @endif
          </a>
        ...
        @hasanyrole('admin_qc|admin_qc_flange|admin_qc_fitting|admin_hr|admin_k3')
        <a href="{{ route('approval_log') }}"
          class="px-2 py-1 {{ request()->routeIs('approval_log') ? 'font-semibold border-b-2 border-indigo-600' : 'text-gray-700 dark:text-gray-300' }}">
          Approval Log
        </a>
        @endhasanyrole

        @hasanyrole('director|kabag_qc|manager_hr|admin_qc|admin_qc_flange|admin_qc_fitting|admin_hr|admin_k3')
    ...
    @if(session('ok'))
      <div class="p-3 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100 rounded mb-4" role="status"
        aria-live="polite">
        {{ session('ok') }}
      </div>
    @endif
